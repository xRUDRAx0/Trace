...
export default function Execution() {
  ...
  useEffect(() => {
    ...
    const fetchRun = async () => {
      try {
        const res = await fetch(`http://localhost:3001/api/executions/${runId}`);
  ...
  const handleApprove = async () => {
    if (!runId) return;
    try {
      await fetch(`http://localhost:3001/api/executions/${runId}/approve`, { method: 'POST' });
    } catch (e) {
  ...
  const handleCancel = async () => {
    if (!runId) return;
    try {
      await fetch(`http://localhost:3001/api/executions/${runId}/cancel`, { method: 'POST' });
    } catch (e) {
